...
- (void)touchesEnded:(NSSet*)touches withEvent:(UIEvent*)event {
  star_.center = CGPointMake( -100, -100 ); //< 그림이 처음에 있는 위치

  [UIView beginAnimations:nil context:NULL]; //< 애니메이션 블록의 시작
  [UIView setAnimationDuration:1.0]; //< １회 애니메이션을 1.0초로 설정한다
  [UIView setAnimationDelay:3.0]; //< 3초 지나고 나서 애니메이션을 시작한다
  [UIView setAnimationRepeatCount:10.0]; //< 애니메이션을 10회 반복한다
  star_.center = CGPointMake( 420, 400 ); //< 그림 이동이 종료될 위치를 설정
